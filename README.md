...
- キャンバスリモコンにある [新規ページを追加] をクリックすると、リモコンにページを追加します。ひとつのリモコンの最大ページ数は 5 です。
- キャンバスリモコン上部のテキストボックスで、リモコンの名前を設定できます。リモコンの名前は必ず設定する必要があります。

###### リモコンの保存
1. リモコン編集画面の左上にある [完了] ボタンを押します。
...
2. この時点では、HUIS 上ではリモコンの削除は行われていません。Home 画面の左上にある [変更を HUIS に反映させる] ボタンを押して同期を行ったときに、削除が反映されます。
